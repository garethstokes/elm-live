 ...
 *
 * This function takes in a directory and a file name, returning the joined
 * path
 **/
...
function getSources_ (packageFilePath) {
  try {
    ...
    if (sourceDirs !== undefined) {
      return [].concat.apply(
        [],
        sourceDirs.map(dir => defaultElmDirs.map(file => join(dir, file)))
      )
    } else {
