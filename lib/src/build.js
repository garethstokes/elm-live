...
const { PROCESS_SUCCESS, merge, update, readFile, writeFile } = require('./utils')
const { elmNotFound, cssNotFound, errorMsg, compileError, buildError, buildSuccess } = require('./messages')
...
/**
 * runCss :: Model -> Async Error Model
 *
 * TODO: comment here
 **/
const runCss = model => Async((reject, resolve) => {
  if (model.css === '') {
  ...
  const proc = spawn(model.css, model.cssArgs.split(' '))

  proc.stderr.on('data', data => update(result, { data: result.data + data }))

  proc.on('error', function (error) {
    if (error.name === 'ENOENT') {
      reject(cssNotFound(model.css))
    } else {
      reject(errorMsg(model.css, error, model.recover))
    }
  })
